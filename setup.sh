...
docker run -d --name dokUniWeb -p 0.0.0.0:8000:8000 -e DOCK_IN_ORG=${DOCK_IN_ORG} -e DOCK_IN_BUCKET=${DOCK_IN_BUCKET} -e DOCK_IN_TOKEN=${DOCK_IN_TOKEN} myapp > /dev/null
